...
interface AppState {
  ...
  resetBattle: () => void;
  calculateScore: (outfit: SavedOutfit) => number;
}
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const calculateScore = (outfit: SavedOutfit) => {
    let score = 0;
    if (outfit.topId) score += Math.floor(Math.random() * 30) + 20;
    if (outfit.bottomId) score += Math.floor(Math.random() * 30) + 20;
    if (outfit.outerId) score += Math.floor(Math.random() * 30) + 20;
    return score;
  };
