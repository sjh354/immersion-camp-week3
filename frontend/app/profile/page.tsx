...
function DeckSlot({
  ...
  return (
    <div
      ref={dropRef}
      className={`relative bg-white rounded-2xl border-5 shadow-[6px_6px_0px_rgba(0,0,0,0.3)] p-6 transition-all ${className ?? ""} ${
        isOver && canDrop
          ...
          <div>
            ...
            <div className="group relative flex items-start">
              <DeckSlot
                ...
                deck={deck}
                onDrop={handleDrop}
                onRemove={() => handleRemoveFromDeck(0)}
                className="w-72 md:w-80 transition-transform duration-300 z-10 group-hover:-translate-x-2"
              />
              <DeckSlot
                index={1}
                outfit={deck[1]}
                deck={deck}
                onDrop={handleDrop}
                onRemove={() => handleRemoveFromDeck(1)}
                className="w-72 md:w-80 -ml-20 md:-ml-28 transition-transform duration-300 group-hover:ml-4"
              />
            </div>
          </div>
...
export default function ProfileRoute() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        const [profileRes, outfitsRes, deckRes] = await Promise.all([
          fetchWithAuth("/members/me"),
          fetchWithAuth("/outfits"),
          fetchWithAuth("/battle/deck"),
        ]);
        if (!profileRes.ok) {
          return;
        }
        ...
        const deckData = deckRes.ok
          ? ((await deckRes.json()) as {
              outfit1Id?: number | null;
              outfit2Id?: number | null;
            })
          : null;
        ...
        const deckOutfits = [deckData?.outfit1Id, deckData?.outfit2Id].map(
          (id) => (id ? outfits.find((item) => item.id === id) : undefined),
        );
        ...
        updateDeck(1, deckOutfits[0]);
        updateDeck(2, deckOutfits[1]);
      } catch {
        // ignore for now
      }
    };
  ...
  const handleUpdateDeck = async (
    ...
    updateDeck(slot, outfit);
    const nextOutfit1Id = (slot === 1 ? outfit?.id : deckSlot1?.id) ?? null;
    const nextOutfit2Id = (slot === 2 ? outfit?.id : deckSlot2?.id) ?? null;
    await fetchWithAuth("/battle/deck", {
      method: "PUT",
      body: JSON.stringify({
        outfit1Id: nextOutfit1Id,
        outfit2Id: nextOutfit2Id,
      }),
    });
  };
