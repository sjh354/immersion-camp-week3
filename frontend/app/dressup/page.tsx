...
function DressUpPageContent({
  ...
  const [clothesPage, setClothesPage] = useState(0);
  const clothesPageRef = useRef(0);
  const isLoadingClothesRef = useRef(false);
  const pendingScrollTopRef = useRef(0);
  const [hasMoreClothes, setHasMoreClothes] = useState(true);
  const clothesScrollRef = useRef<HTMLDivElement | null>(null);
  ...
  const loadClothes = async (page: number, replace: boolean) => {
    if (isLoadingClothesRef.current || isLoadingClothes) return;
    isLoadingClothesRef.current = true;
    setIsLoadingClothes(true);
    setClothesError(null);
    try {
      const pageSize = 24;
      const keyword = searchQuery.trim();
      let endpoint = `/clothes?page=${page}&size=${pageSize}`;
      ...
      const data = (await response.json()).content as Array<{
        id: number;
        name: string;
      ...
      if (keyword) {
        ...
        setHasMoreClothes(mapped.length === pageSize);
        setClothesPage(page);
        clothesPageRef.current = page;
        if (!replace && clothesScrollRef.current) {
          const target = clothesScrollRef.current;
      ...
      isLoadingClothesRef.current = false;
    }
  };
  const handleClothesScroll = (event: React.UIEvent<HTMLDivElement>) => {
    if (isLoadingClothes || !hasMoreClothes) return;
    if (searchQuery.trim()) return;
    const target = event.currentTarget;
    if (target.scrollTop + target.clientHeight >= target.scrollHeight - 200) {
      pendingScrollTopRef.current = target.scrollTop;
      loadClothes(clothesPageRef.current + 1, false);
    }
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      if (!outfit.top || !outfit.bottom) {
        alert("상의와 하의는 모두 선택해야 합니다.");
        return;
      }
      const sourceResponse = await fetch(resultImageUrl);
      if (!sourceResponse.ok) {
        throw new Error("preview download failed");
      }
      const blob = await sourceResponse.blob();
      const file = new File([blob], "outfit_preview.png", {
          ...
          >
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                ...
                {!isLoadingClothes && hasMoreClothes && !searchQuery.trim() && (
                  <p className="col-span-2 text-center text-xs font-bold text-gray-500">
                    아래로 스크롤하면 더 불러옵니다.
                  </p>
                )}
              </div>
