...
export default function LandingRoute() {
  ...
  const { deckSlot1, deckSlot2, setSavedOutfits, updateDeck, token } =
    useAppState();
  ...
  useEffect(() => {
    ...
    const loadDeck = async () => {
      try {
        ...
        const data = (await profileRes.json()) as { nickname?: string };
        if (data.nickname) {
        ...
        const outfitsPayload = outfitsRes.ok
          ...
          : [];
        const outfits = outfitsPayload.map((item) => ({
          id: item.id,
          name: item.name,
    ...
    loadDeck();
  }, [deckSlot1, deckSlot2, setSavedOutfits, token, updateDeck]);

  if (!skipAuth && !token && !authChecked) {
    return (
      <div className="h-screen w-screen bg-gradient-to-br from-purple-200 via-pink-200 to-cyan-200 flex items-center justify-center">
        <div className="bg-white px-6 py-4 rounded-xl border-4 border-black font-black text-purple-600 shadow-[6px_6px_0px_rgba(0,0,0,0.3)]">
          로그인 확인 중...
        </div>
      </div>
    );
